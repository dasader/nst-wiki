...
표는 "구조를 가지느냐"가 아니라 **"이 표에 SQL을 던질 일이 있느냐(집계·필터·조인·문서간 비교)"** 를 기준으로 세 티어 중 하나로 라우팅한다. 모든 표를 관계형 스키마에 맞추려는 시도는 표 형태 공간이 열려 있어 수렴하지 않는다(12.4절) — 질의 가치 있는 표만 정형화하고 나머지는 손실 없이 보존한다.

- **티어 1 — 엔티티 테이블 (5.2절 고정 스키마):** 반복 등장하는 고가치 엔티티(기술·사업·부처·예산·정책이벤트). LLM이 표 컬럼을 이 테이블 컬럼에 매핑한다. 매핑 결과는 대상 테이블·컬럼 대응표·신뢰도 점수로 구성된다.
- **티어 2 — 지표 테이블 (`metrics`, 롱포맷):** 티어 1에 안 맞지만 `(대상, 지표, 연도, 값)` 형태로 환원되는 수치 표(연도별 예산·목표·인력 등). 표 하나당 새 스키마를 만들지 않고 하나의 긴 테이블이 흡수하면서 질의 가능성을 유지한다. **`metric_name`은 통제 어휘로 정규화**해야 하며(그렇지 않으면 EAV로 붕괴), 정규화는 매핑 단계에서 LLM이 수행하되 어휘 목록은 사람이 관리한다.
- **티어 3 — md 인라인 폴백:** 위 둘에 안 맞는 불규칙 표는 `staging_tables`에 원본(JSONB)으로 보존하는 **동시에** 관련 위키 페이지에 마크다운 표로 그대로 인라인한다. 질의는 안 되지만 표시·벡터 검색은 되므로 정보 손실이 0이다.

공통 처리: **정규화**(셀 병합 해제, 반복 헤더 제거, 타입 변환) → **staging 적재**(티어 1·2는 매핑 신뢰도가 임계값(기본 0.8, 설정 가능) 이상이면 `staging` 스키마 동일 구조 테이블에, 미만이거나 대응이 없으면 티어 3으로) → **승인 시 upsert**(본 테이블과 비교해 삽입/갱신, 충돌은 승인 화면에 표출).

**LLM은 DDL을 생성하지 않는다.** 스키마 확장은 **수요 기반**으로만 한다: `staging_tables`에 반복 등장하는(임계 횟수 이상) 표 형태를 대시보드가 제안하면, 사람이 그때 일괄로 마이그레이션 파일을 작성해 티어 1로 승격한다. 매 문서를 좇지 않으므로 스키마가 안정된 집합으로 수렴한다.
...
-- 롱포맷 지표 테이블 (티어 2): 엔티티 테이블에 안 맞는 (대상,지표,연도,값) 수치 표 흡수.
-- 표마다 새 스키마를 만들지 않으면서 집계·비교 질의를 유지하는 "좁은 허리".
CREATE TABLE metrics (
    id SERIAL PRIMARY KEY,
    entity VARCHAR(200) NOT NULL,      -- 지표 대상 (기술명·사업명·분야 등)
    metric_name VARCHAR(100) NOT NULL, -- 통제 어휘로 정규화 (예: 예산, 목표_세계최초기술수, 인력)
...
-- 매핑 불가 표 보존 (티어 3): 원본을 여기 남기고 위키 페이지엔 md 표로 인라인
CREATE TABLE staging_tables (
    ...
    status VARCHAR(20) DEFAULT 'needs_review',  -- needs_review/mapped/inlined/discarded
    created_at TIMESTAMP DEFAULT NOW()
);
...
### 12.4 왜 고정 스키마인가 (그리고 "모든 표를 DB에"가 환상인 이유)

LLM이 표마다 테이블을 생성·확장하면 유사 테이블이 난립하고(스키마 드리프트) Text-to-SQL 품질이 함께 무너진다. 핵심 테이블을 사람이 관리하고 LLM은 매핑만 하게 제한한다.

단, **"모든 표를 고정 스키마에 맞춘다"는 목표 자체가 환상이다.** 정책문서의 표 형태 공간은 열려 있어(예산·일정·조직·비교매트릭스…), 스키마로 좇으면 (a) 끝없는 마이그레이션이거나 (b) 뭐든 담는 범용 스키마(EAV/JSON blob)로 귀결된다. 후자는 DB에 넣었을 뿐 조인·집계의 이점을 잃은 "구조화의 환상"이다.

따라서 판별 기준은 "구조를 가지나"가 아니라 **"질의 가치가 있나(누가 SQL로 물을 일이 있나)"** 다. 질의 가치 있는 유형은 도메인 안에서 소수이고 반복되므로(기술·사업·부처·예산) 고정 스키마가 오히려 옳은 베팅이다. 수치 꼬리의 대부분은 롱포맷 `metrics` 테이블이 흡수하고(4.4 티어 2), 진짜 불규칙한 표는 md 인라인으로 손실 없이 보존한다(티어 3). 스키마는 예측이 아니라 **반복 증거가 쌓일 때만 수요 기반으로** 확장하므로 안정된 집합으로 수렴한다 — 이것이 "표를 결국 DB로"라는 목표를 환상에 빠지지 않고 실제로 이루는 길이다.
